...
@pytest.fixture
def mock_db():
    """Provide a mongomock database for testing auth."""
...
class TestTeamsRoute:
    def test_standings_page_renders(self, logged_in_client):
        teams = [
    ...
    def test_teams_sorted_by_standing(self, logged_in_client):
        teams = [
    ...
    def test_team_record_displayed(self, logged_in_client):
    ...
    def test_team_record_with_ties(self, logged_in_client):
    ...
    def test_points_displayed(self, logged_in_client):
    ...
    def test_username_displayed_in_nav(self, logged_in_client):
        teams = [make_team()]
        with patch("app.get_league", return_value=make_league(teams)):
            response = logged_in_client.get("/")
        html = response.data.decode()
...
class TestRosterRoute:
    ...
    def test_roster_page_renders(self, logged_in_client):
        team = self._team_with_roster()
        with patch("app.get_league", return_value=make_league([team])):
            response = logged_in_client.get("/team/1")
        assert response.status_code == 200
        html = response.data.decode()
        assert team.team_name in html

    def test_invalid_team_returns_404(self, logged_in_client):
        team = make_team(team_id=1)
        with patch("app.get_league", return_value=make_league([team])):
            response = logged_in_client.get("/team/999")
        assert response.status_code == 404

    def test_starters_section_displayed(self, logged_in_client):
        team = self._team_with_roster()
        with patch("app.get_league", return_value=make_league([team])):
            response = logged_in_client.get("/team/1")
        html = response.data.decode()
    ...
    def test_bench_section_displayed(self, logged_in_client):
        team = self._team_with_roster()
        with patch("app.get_league", return_value=make_league([team])):
            response = logged_in_client.get("/team/1")
        html = response.data.decode()
    ...
    def test_ir_section_displayed(self, logged_in_client):
        team = self._team_with_roster()
        with patch("app.get_league", return_value=make_league([team])):
            response = logged_in_client.get("/team/1")
        html = response.data.decode()
    ...
    def test_starters_sorted_by_position(self, logged_in_client):
        team = self._team_with_roster()
        with patch("app.get_league", return_value=make_league([team])):
            response = logged_in_client.get("/team/1")
        html = response.data.decode()
    ...
    def test_op_slot_displays_as_qb(self, logged_in_client):
        roster = [
    ...
    def test_rb_wr_te_slot_displays_as_flex(self, logged_in_client):
    ...
    def test_injury_status_shown(self, logged_in_client):
        roster = [
    ...
    def test_active_injury_status_hidden(self, logged_in_client):
        roster = [
    ...
    def test_player_points_displayed(self, logged_in_client):
    ...
    def test_high_scorer_highlighted(self, logged_in_client):
    ...
    def test_low_scorer_not_highlighted(self, logged_in_client):
    ...
    def test_back_link_present(self, logged_in_client):
    ...
    def test_empty_roster_no_sections(self, logged_in_client):
        team = make_team(team_id=1, roster=[])
        with patch("app.get_league", return_value=make_league([team])):
            response = logged_in_client.get("/team/1")
        html = response.data.decode()
